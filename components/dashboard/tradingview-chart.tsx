...
export function TradingViewChart({ ticker }: ChartProps) {
  ...
  useEffect(() => {
    ...
    return () => { chart.remove(); chartRef.current = null; candleRef.current = null; volumeRef.current = null; indicatorRefs.current = {} }
  }, [])
  ...
  const drawIndicators = useCallback((bars: Bar[], active: Set<string>) => {
    ...
    // ─── OSCILLATOR / SEPARATE PANE INDICATORS (rendered as overlays for simplicity) ───
    // Note: Lightweight Charts doesn't support multiple panes easily, so these render as small overlays
    if (active.has('rsi')) { const s = addLine('#a855f7', 1); s.setData(toLineData(calcRSI(closes))); store('rsi', s) }
    if (active.has('stoch')) {
      const { k, d } = calcStochastic(bars)
      const sK = addLine('#3b82f6', 1); sK.setData(toLineData(k)); store('stoch', sK)
      const sD = addLine('#f59e0b', 1); sD.setData(toLineData(d)); store('stoch', sD)
    }
    if (active.has('stochrsi')) {
      const { k, d } = calcStochRSI(closes)
      const sK = addLine('#8b5cf6', 1); sK.setData(toLineData(k)); store('stochrsi', sK)
      const sD = addLine('#ec4899', 1); sD.setData(toLineData(d)); store('stochrsi', sD)
    }
    if (active.has('macd')) {
      const { macd, signal } = calcMACD(closes)
      const sM = addLine('#22c55e', 1); sM.setData(toLineData(macd)); store('macd', sM)
      const sS = addLine('#f59e0b', 1); sS.setData(toLineData(signal)); store('macd', sS)
    }
    if (active.has('cci')) { const s = addLine('#f59e0b', 1); s.setData(toLineData(calcCCI(bars))); store('cci', s) }
    if (active.has('williams')) { const s = addLine('#ec4899', 1); s.setData(toLineData(calcWilliamsR(bars))); store('williams', s) }
    if (active.has('roc')) { const s = addLine('#06b6d4', 1); s.setData(toLineData(calcROC(closes))); store('roc', s) }
    if (active.has('momentum')) { const s = addLine('#14b8a6', 1); s.setData(toLineData(calcMomentum(closes))); store('momentum', s) }
    if (active.has('trix')) { const s = addLine('#f97316', 1); s.setData(toLineData(calcTRIX(closes))); store('trix', s) }
    if (active.has('uo')) { const s = addLine('#84cc16', 1); s.setData(toLineData(calcUltimateOscillator(bars))); store('uo', s) }
    if (active.has('ao')) { const s = addLine('#22c55e', 1); s.setData(toLineData(calcAwesomeOscillator(bars))); store('ao', s) }
    if (active.has('dpo')) { const s = addLine('#0ea5e9', 1); s.setData(toLineData(calcDPO(closes))); store('dpo', s) }

    // ─── VOLUME INDICATORS ───
    if (active.has('obv')) { const s = addLine('#3b82f6', 1); s.setData(toLineData(calcOBV(bars))); store('obv', s) }
    if (active.has('mfi')) { const s = addLine('#22c55e', 1); s.setData(toLineData(calcMFI(bars))); store('mfi', s) }
    if (active.has('cmf')) { const s = addLine('#f59e0b', 1); s.setData(toLineData(calcCMF(bars))); store('cmf', s) }
    if (active.has('ad')) { const s = addLine('#a855f7', 1); s.setData(toLineData(calcAD(bars))); store('ad', s) }
    if (active.has('vpt')) { const s = addLine('#ec4899', 1); s.setData(toLineData(calcVPT(bars))); store('vpt', s) }
    if (active.has('eom')) { const s = addLine('#14b8a6', 1); s.setData(toLineData(calcEOM(bars))); store('eom', s) }
    if (active.has('fi')) { const s = addLine('#f97316', 1); s.setData(toLineData(calcForceIndex(bars))); store('fi', s) }

    // ─── VOLATILITY INDICATORS ───
    if (active.has('atr')) { const s = addLine('#f59e0b', 1); s.setData(toLineData(calcATR(bars))); store('atr', s) }
    if (active.has('natr')) { const s = addLine('#fbbf24', 1); s.setData(toLineData(calcNATR(bars))); store('natr', s) }
    if (active.has('tr')) { const s = addLine('#f97316', 1); s.setData(toLineData(calcTrueRange(bars))); store('tr', s) }
    if (active.has('stddev')) { const s = addLine('#a855f7', 1); s.setData(toLineData(calcStdDev(closes))); store('stddev', s) }
    if (active.has('hv')) { const s = addLine('#8b5cf6', 1); s.setData(toLineData(calcHistoricalVolatility(closes))); store('hv', s) }

    // ─── TREND INDICATORS ───
    if (active.has('adx')) {
      const { adx } = calcADX(bars)
      const s = addLine('#22c55e', 1); s.setData(toLineData(adx)); store('adx', s)
    }
    if (active.has('dmi')) {
      const { pdi, ndi } = calcADX(bars)
      const sP = addLine('#22c55e', 1); sP.setData(toLineData(pdi)); store('dmi', sP)
      const sN = addLine('#dc2626', 1); sN.setData(toLineData(ndi)); store('dmi', sN)
    }
    if (active.has('aroon')) {
      const { up, down } = calcAroon(bars)
      const sU = addLine('#22c55e', 1); sU.setData(toLineData(up)); store('aroon', sU)
      const sD = addLine('#dc2626', 1); sD.setData(toLineData(down)); store('aroon', sD)
    }
    if (active.has('aroonosc')) { const s = addLine('#fbbf24', 1); s.setData(toLineData(calcAroonOsc(bars))); store('aroonosc', s) }
    if (active.has('vi')) {
      const { vip, vim } = calcVortex(bars)
      const sP = addLine('#22c55e', 1); sP.setData(toLineData(vip)); store('vi', sP)
      const sM = addLine('#dc2626', 1); sM.setData(toLineData(vim)); store('vi', sM)
    }
