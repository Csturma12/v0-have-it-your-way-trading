...
async function fetchInsiderData(ticker: string) {
  try {
    ...
    return { recentTrades: trades.length, netDirection, totalValue, source: 'unusual_whales' }
  } catch (err) {
    console.error(`[Aggregate] Insider fetch error for ${ticker}:`, err)
    return { recentTrades: 0, netDirection: 'none' as const, totalValue: 0, source: 'unavailable' }
  }
}
