            ...
            <div className="mt-4 space-y-4 pt-4 border-t border-border">
              ...
              <div className="p-4 rounded-lg border border-border bg-muted/30">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() => updateConfig({ follow_ai_recommendations: !config.follow_ai_recommendations })}
                    className={`w-10 h-5 rounded-full transition-colors ${
                      config.follow_ai_recommendations ? 'bg-green-500' : 'bg-muted'
                    }`}
                  >
                    <div className={`w-4 h-4 rounded-full bg-white transition-transform ${
                      config.follow_ai_recommendations ? 'translate-x-5' : 'translate-x-0.5'
                    }`} />
                  </button>
                </div>

                <div className="flex items-center justify-between">
                  <span className="text-sm">Require AI consensus</span>
                  <button
                    onClick={() => updateConfig({ require_consensus: !config.require_consensus })}
                    className={`w-10 h-5 rounded-full transition-colors ${
                      config.require_consensus ? 'bg-green-500' : 'bg-muted'
                    }`}
                  >
                    <div className={`w-4 h-4 rounded-full bg-white transition-transform ${
                      config.require_consensus ? 'translate-x-5' : 'translate-x-0.5'
                    }`} />
                  </button>
                </div>

                <div className="flex items-center justify-between">
                  <span className="text-sm">Use Claude signals</span>
                  <button
                    onClick={() => updateConfig({ use_claude_signals: !config.use_claude_signals })}
                    className={`w-10 h-5 rounded-full transition-colors ${
                      config.use_claude_signals ? 'bg-green-500' : 'bg-muted'
                    }`}
                  >
                    <div className={`w-4 h-4 rounded-full bg-white transition-transform ${
                      config.use_claude_signals ? 'translate-x-5' : 'translate-x-0.5'
                    }`} />
                  </button>
                </div>

                <div className="flex items-center justify-between">
                  <span className="text-sm">Use OpenAI signals</span>
                  <button
                    onClick={() => updateConfig({ use_openai_signals: !config.use_openai_signals })}
                    className={`w-10 h-5 rounded-full transition-colors ${
                      config.use_openai_signals ? 'bg-green-500' : 'bg-muted'
                    }`}
                  >
                    <div className={`w-4 h-4 rounded-full bg-white transition-transform ${
                      config.use_openai_signals ? 'translate-x-5' : 'translate-x-0.5'
                    }`} />
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <h3 className="text-sm font-mono text-muted-foreground uppercase tracking-wide">Risk Limits</h3>
                
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span>Max position size</span>
                    <span className="font-mono">${riskLimits.max_position_size}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span>Max total exposure</span>
                    <span className="font-mono">${riskLimits.max_total_exposure}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span>Daily loss limit</span>
                    <span className="font-mono">${riskLimits.daily_loss_limit}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span>Max concurrent positions</span>
                    <span className="font-mono">{riskLimits.max_concurrent_positions}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span>Min confidence to trade</span>
                    <span className="font-mono">{riskLimits.min_confidence_score}%</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span>Stop loss</span>
                    <span className="font-mono">{riskLimits.stop_loss_percent}%</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span>Take profit</span>
                    <span className="font-mono">{riskLimits.take_profit_percent}%</span>
                  </div>
                </div>
              </div>
            </div>
